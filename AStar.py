        ...
        for i in range(0, len(nodes)):  # Add all accessible states to the beginning of the frontier
            if len(self.frontier) == 0:
                ...
                for j in range(0, len(self.frontier)):
                    if (nodes[i].distance_to_nearest_goal + nodes[i].cost_from_initial_node) <= (self.frontier[j].distance_to_nearest_goal + self.frontier[j].cost_from_initial_node):  # Sort according to cost
